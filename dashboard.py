...
def tracking_diagnostics(row):
    clicks, lpv, vc, atc, ic, purchases = [safe_num(row.get(x)) for x in ['clicks','landing_page_view','view_content','add_to_cart','initiate_checkout','purchases']]
...
def show_growth_center(df):
    ...
    cols = st.columns(3)
    cards = [(cols[0], 'Scale Now', scale, 'alert-good'), (cols[1], 'Fix First', fix, 'alert-info'), (cols[2], 'Pause / Cut', pause, 'alert-hot')]
    for col, title, subset, cls in cards:
        if subset.empty:
...
def show_charts(level_df):
    st.markdown('<div class="table-title">Performance Charts</div><div class="help-text">Quick visual read for spend, ROAS, tracking health, and score.</div>', unsafe_allow_html=True)
    ...
    with c2: st.bar_chart(chart_df[['manual_roas','score','tracking_health_score']])
...
try:
    ...
    if df.empty:
        ...
        st.stop()
    if selected_account_id and 'account_id' in df.columns: df = df[df['account_id'] == selected_account_id]
    if df.empty:
    ...
    if window_df.empty:
        ...
        else: show_table(f'{window_choice} — {level_choice} {view_choice}', 'الاسم متثبت كـ index عشان يفضل ظاهر وانت بتسكرول أفقياً.', filtered, decision_cols, name_col=name_col)
        if not filtered.empty:
            ...
            st.download_button('Export filtered action plan CSV', filtered[decision_cols].to_csv(index=True), 'action_plan.csv', 'text/csv')
